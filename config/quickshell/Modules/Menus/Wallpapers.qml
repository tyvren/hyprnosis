...
Scope {
    ...
    property string wallpaperDir: Quickshell.env("HOME") + "/.config/hyprnosis/wallpapers/" + root.folderName
    ...
    Process {
        ...
        command: ["find", "-L", root.wallpaperDir, "-type", "f", "!", "-name", ".*"]
        running: true
                ...
                PathView {
                    ...
                    delegate: Item {
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    Config.updateWallpaper(delegateItem.modelData);
                    ...
                    Keys.onPressed: event => {
                        if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                            let selectedWall = model[currentIndex];
                            Config.updateWallpaper(selectedWall);
                            Quickshell.execDetached([
                                Quickshell.env("HOME") + "/.config/hyprnosis/modules/style/wallpaper_changer.sh",
                                selectedWall
                            ]);
